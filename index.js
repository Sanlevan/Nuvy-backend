...
app.get('/v1/passes/:pId/:sN', async (req,res) => {
    const {data:c} = await supabase.from('clients').select('*, boutiques(nom)').eq('serial_number', req.params.sN).single()
    const buffer = await generatePassBuffer(c?c.tampons:0, c?c.nom:'Client', req.params.sN, c?.boutiques?.nom)
