...
// ==========================================
// 🏢 MULTI-SITE : GESTION DES RÉSEAUX
// ==========================================

// CEO : Créer un réseau
app.post('/admin/create-reseau', async (req, res) => {
...
// CEO : Lister les réseaux
app.get('/admin/reseaux', async (req, res) => {
    if (req.headers['x-ceo-key'] !== MASTER_CEO_KEY) return res.status(403).json({ error: "Accès refusé" });
    const { data, error } = await supabase
        .from('reseaux')
        .select('id, nom, slug, username, created_at');
    if (error) return res.status(500).json({ error: error.message });
    res.json(data || []);
});

// CEO : Rattacher une boutique à un réseau
app.put('/admin/boutique/:id/reseau', async (req, res) => {
    if (req.headers['x-ceo-key'] !== MASTER_CEO_KEY) return res.status(403).json({ error: "Accès refusé" });
...
// LOGIN réseau (super admin commerçant)
app.post('/auth/login-reseau', limiterLogin, async (req, res) => {
    const { user, pass } = req.body;
    const { data: reseau } = await supabase
        .from('reseaux')
        .select('id, slug, nom, password')
        .eq('username', user)
        .maybeSingle();
...
// GET : Boutiques du réseau
app.get('/reseau/:id/boutiques', verifyAuthReseau, async (req, res) => {
    if (String(req.auth.reseauId) !== String(req.params.id)) {
        return res.status(403).json({ error: "Accès interdit." });
    }
    const { data, error } = await supabase
        .from('boutiques')
        .select('id, nom, slug, categorie, plan, created_at, adresse, color_text')
        .eq('reseau_id', req.params.id)
        .order('nom');
    if (error) return res.status(500).json({ error: error.message });
    res.json(data || []);
});

// GET : Stats consolidées du réseau
app.get('/reseau/:id/stats', verifyAuthReseau, async (req, res) => {
    if (String(req.auth.reseauId) !== String(req.params.id)) {
        return res.status(403).json({ error: "Accès interdit." });
    }
    try {
        ...
        if (!boutiques || boutiques.length === 0) {
            return res.json({ totalClients: 0, totalScans30j: 0, boutiquesActives: 0, topClients: [], scansParBoutique: [] });
        }
        ...
        const [
            ...
            supabase.from('clients').select('nom, telephone, total_historique, boutique_id').in('boutique_id', boutiqueIds).order('total_historique', { ascending: false }).limit(10)
        ]);

        // Boutiques actives (au moins 1 scan dans les 7 derniers jours)
        const activesIds = new Set(visites7j?.map(v => v.boutique_id) || []);
        const boutiquesActives = activesIds.size;

        // Scans par boutique (30j)
        const scanCount = {};
        visites30j?.forEach(v => { scanCount[v.boutique_id] = (scanCount[v.boutique_id] || 0) + 1; });
        const scansParBoutique = boutiques.map(b => ({
            nom: b.nom,
            id: b.id,
            scans: scanCount[b.id] || 0
        })).sort((a, b) => b.scans - a.scans);

        // Top clients réseau (cumul par user_id si possible, sinon par téléphone)
        const clientsParTel = {};
        topClientsRaw?.forEach(c => {
            const tel = c.telephone;
            if (!clientsParTel[tel]) {
                clientsParTel[tel] = { nom: c.nom, telephone: tel, total: 0, boutiques: 0 };
            }
            clientsParTel[tel].total += (c.total_historique || 0);
            clientsParTel[tel].boutiques += 1;
        });
        const topClients = Object.values(clientsParTel)
        ...
        visites30j?.forEach(v => {
            const j = v.created_at.split('T')[0];
            if (scansParJour[j] !== undefined) scansParJour[j]++;
        });
...
// GET : Clients cumulés réseau (avec déduplication par téléphone)
app.get('/reseau/:id/clients', verifyAuthReseau, async (req, res) => {
    if (String(req.auth.reseauId) !== String(req.params.id)) {
        return res.status(403).json({ error: "Accès interdit." });
    }
    try {
        ...
        const { data: clients } = await supabase
            .from('clients')
            .select('id, nom, telephone, tampons, recompenses, total_historique, last_visit, boutique_id')
            .in('boutique_id', boutiqueIds)
            .order('total_historique', { ascending: false });

        // Dédupliquer par téléphone pour avoir la vue réseau
        const clientsReseau = {};
        clients?.forEach(c => {
            const tel = c.telephone;
            if (!clientsReseau[tel]) {
                clientsReseau[tel] = {
                    nom: c.nom,
                    telephone: tel,
                    total_reseau: 0,
        ...
        const result = Object.values(clientsReseau)
            .sort((a, b) => b.total_reseau - a.total_reseau);
...
// Page dashboard réseau
app.get('/reseau-dashboard', (req, res) => {
    res.sendFile(path.resolve(__dirname, 'reseau-dashboard.html'));
});

app.get('/reseau-login', (req, res) => {
    res.sendFile(path.resolve(__dirname, 'reseau-login.html'));
});

// Réseau : se connecter au dashboard d'une boutique spécifique
app.post('/reseau/login-as-boutique', verifyAuthReseau, async (req, res) => {
    const { boutiqueId, reseauId } = req.body;
    if (String(req.auth.reseauId) !== String(reseauId)) {
        return res.status(403).json({ error: "Accès interdit." });
    }
    const { data: boutique } = await supabase
        .from('boutiques')
...
app.get('/reseau/:id/stats', verifyAuthReseau, async (req, res) => {
    ...
    try {
        ...
        visites30j?.forEach(v => { scanCount[v.boutique_id] = (scanCount[v.boutique_id] || 0) + 1; });
        const scansParBoutique = boutiques.map(b => ({
            nom: b.nom, id: b.id, scans: scanCount[b.id] || 0
        })).sort((a, b) => b.scans - a.scans);

        // Top clients cumulés par téléphone
        const clientsParTel = {};
        topClientsRaw?.forEach(c => {
            const tel = c.telephone;
            if (!clientsParTel[tel]) {
...
// Réseau : clients cumulés dédupliqués
app.get('/reseau/:id/clients', verifyAuthReseau, async (req, res) => {
    if (String(req.auth.reseauId) !== String(req.params.id)) {
        return res.status(403).json({ error: "Accès interdit." });
    }
    try {
        ...
        clients?.forEach(c => {
            ...
            if (!clientsReseau[tel]) {
                clientsReseau[tel] = {
                    nom: c.nom, telephone: tel,
                    total_reseau: 0, boutiques_visitees: [], derniere_visite: null
                };
            }
            clientsReseau[tel].total_reseau += (c.total_historique || 0);
            clientsReseau[tel].boutiques_visitees.push(boutiqueMap[c.boutique_id]);
...
// Super admin : accéder au dashboard d'une boutique de son réseau
app.post('/reseau/login-as-boutique', verifyAuthReseau, async (req, res) => {
    const { boutiqueId, reseauId } = req.body;
    if (String(req.auth.reseauId) !== String(reseauId)) {
        return res.status(403).json({ error: "Accès interdit." });
